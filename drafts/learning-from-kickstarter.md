...
Having evaluated that to a Clojure REPL in which you've loaded [`http-kit`](TODO) and [`cheshire`](TODO), you can evaluate `(collect! 0)` and wait a very long while. After waiting long enough[^and-probably-restarting], you'll have a file containing around 165 thousand [EDT](TODO - clojure EDT)-encoded kickstarter projects. We don't really need all of the stuff present to learn from it; so once we got the raw data pulled down, we can sift it for interesting bits.
