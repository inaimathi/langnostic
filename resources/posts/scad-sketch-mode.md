...
I can't easily look at those things and visualize the corresponding lines or shapes. This is a problem if I want to create any physical item not derived from simple math. The one part of [`FreeCAD`](https://www.freecad.org/) that I really like is the idea of a "Sketch" interface that lets you draw various 2D shapes that you can then `extrude`/`difference` or transform in various ways in order to get the 3D shapes out of your head and into your computer[^generally-as-a-brief]. So, in the grand tradition of Emacs users and Lispers in general, I'm going to steal it.
...
Anyway, [`scad-sketch-mode`](https://github.com/inaimathi/scad-sketch-mode) exists now. It's a small, keyboard-driven companion 2D editor to `scad-mode` hosted inside Emacs. Soon, it'll be a completely pilfered version of the FreeCAD sketch mode. I've been doing more work in AI-assisted programming lately; currently it's shockingly useful below some boundary and unreliable above it. I intend to continue the AI-assisted experiments, and push that sketch mode further (and submit it to [`melpa`](https://melpa.org/#/)). 
