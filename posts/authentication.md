...
1. [Hash functions](http://en.wikipedia.org/wiki/Hash_function) are tricky to pick, even before you get into [cryptographic hash functions](http://en.wikipedia.org/wiki/Cryptographic_hash_function#Cryptographic_hash_algorithms). In fact, there are a couple of widely-used ones[^such-as-md5-and-sha1] that have been subject to successful attacks. Given that, I'm leaning towards the [SHA-2 algorithms](http://en.wikipedia.org/wiki/SHA-2) which, as of this writing, have not been successfully broken. **DO NOT** read that as "I should use SHA256 from now on". Read it instead as "Before deciding on a hash function, I should check which ones are difficult to break at the time I'm making the decision". That complicates things somewhat, because Erlang's `crypto` only supports MD5 and SHA-1, installing the Erlang SHA256 library seems to be more than trivially difficult.
